...
<td>{{ $data->email }}</td>
<td>{{ $data->name }}</td>
<td>{{ $data->assigned_district}}</td>
<td>{{ $data->assigned_region }}</td>
<td>{{ $data->vehical_name }}</td>
<td>{{ $data->mobile }}</td>
<td><a class="btn btn-primary" href="{{ route('edit.delivery.boy',$data->id) }}">edit</a><a class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this data')" href="{{ route('delete.dboy',$data->id) }}">delete</a></td>
</tr>
</tbody>    
@endforeach()
</table>
