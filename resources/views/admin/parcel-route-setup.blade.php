...
<body>
                                 ...
                                 {{-- <li class="text-center py-2">
                                    <a href="{{ route('view_all_merchant') }}">
                                        <div class="menu-list">
                                         ...
                                         <p class="text-center font-15 text-color-3 mt-1 mb-0">View All Merchants</p>
                                    </a>
                                </li> --}}
                                 <li class="text-center py-2">
                ...
                @if(Session::has('message'))
                   ...
                   <button class="close" type="button" data-dismiss="alert">×</button>
                    <strong>{{ Session::get('error')}}</strong> <div id="Div2" runat="server" ></div>
                      ...
                      @endif()
             {{--  <div class="add_btn">
                  <a class="btn btn-primary" href="{{ route('show-merchant') }}">add</a>

              </div>  
         --}}
    </div>

    <form  action="{{ route('add_parcel_route') }}" method="post" >
        @csrf
    <div class="row">
        <div class="col-lg-6">
            <p style="background-color: red; text-align: center;"> Pickup Zone</p>
            <div class="row">
         <div class="col-lg-6">
            <label> District</label>
           <select class="pickup_dist" name="pickup_district">
            <option value="">Please Select</option>
            @foreach($merchant as $merch)
               <option>{{ $merch->district }}</option>
               @endforeach()
           </select>  
         </div>
         <div class="col-lg-6">
            <label> Region</label><br>
             <select class="pickup_reg" name="pickup_region">
                <option value="">Please Select</option>
                @foreach($merchant as $merchant)
                 <option> {{ $merchant->region?? 'null' }} </option>
               @endforeach()
             </select>
         </div>
     </div>
         ...
         <div class="col-lg-6">
           <p style="background-color: green; text-align: center; color: white;">Drop zone</p> 
           <div class="row">
         <div class="col-lg-6">
            <label> District</label>
           <select class="drop_dist" name="drop_district">
            <option value="">Please Select</option>
            @foreach($parcel as $parc)
            
               <option>{{ $parc->district }}</option>
               @endforeach()
           </select>  
         </div>
         <div class="col-lg-6">
            <label> Region</label><br>
             <select class="drop_reg" name="drop_region">
                <option value="">Please Select</option>
                @foreach($parcel as $parc)
                 <option >{{ $parc->region }}</option>
                @endforeach()
             </select>
         </div>
   ...
   <br>
    ...
    <a  class="btn btn-success add-button-route" style="color:white;">Add Route</a>
 
<div class="row">
<table id="myTable">
    <tbody>
        <tr>
            <td><input id="route-data" type="text" name="route_name[]"></td>
</tr>
    </tbody>
</table>

</div>
<label style="border-radius: 2px; background-color: red;"> Delivery Boy</label>
<select name="user_id">
    <option value="">Please Select</option>
    @foreach($boy as $boy)
    <option value="{{ $boy->id }}">
...
<div class=" container">
    ...
    <table id = "getting-data" class="display">

<thead>
  <tr>
    <th>Pickup Zone (D | R)</th>
    <th>Delivery Zone ( D | R)</th>
    <th>Route</th>
...
<tbody>
      ...
      <?php $route_name = json_decode($data->route) ?>
  <tr>
  
<td>{{ $data->pickup_district }} | {{  $data->pickup_region}}</td>
<td>{{ $data->drop_district }} | {{  $data->drop_region }}</td>
<td><ul>@foreach($route_name as $route_names)<li>{{ $route_names }} </li>@endforeach()</ul></td>
<td>{{ $data->drop_district}} | {{ $data->drop_region }}</td>

<td><a class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this data')" href="{{ route('delete.parcel.setup',$data->id) }}">delete</a></td>
</tr>
@endforeach()
</tbody>    
...
$('.add-button-route').click(function(){
//     //Add row
    table.append('<tr><td><input type="text" name="route_name[]"></td></tr>');
});
