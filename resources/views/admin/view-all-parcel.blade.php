...
<thead>
  <tr>
    <th>SL</th>
    <th>Parcel Code</th>
    <th>Parcel Amount with Delivery Charges</th>
    <th>Business Name</th>
...
<td>{{ $data->amount }}</td>
<td>{{  $data->user->bussiness_name}}</td>
<td>{{ $data->user->mobile }}</td>
<td>{{ $data->user->name}}</td>
<td>{{ $data->deliveryboy['name']?? 'empty'}} </td>
<td>@if($data->status == 0) Pending @elseif($data->status == 1) Accepted @else Rejected @endif</td>
<td>{{ $data->created_at }}</td>
<td>{{ $data->note }}</td>
<td><a class="btn btn-primary" href="{{ route('edit_parcel',$data->id) }}">edit</a><a class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this data')" href="{{ route('delete.parcel',$data->id) }}">delete</a></td>
</tr>
</tbody>    
@endforeach()
</table>
